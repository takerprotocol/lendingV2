...
import IDebtToken from 'abis/IDebtToken.json'
...
export function useDTokenApproval(
  ...
  const erc20ReserveData = useErc20ReserveData()
  const tokenContract = useContract(erc20ReserveData.debtTokenAddress, IDebtToken)
  const [approvalState, setApprovalState] = useState(ApprovalState.NOT_APPROVED)
  const address = useAddress()
  const hasPendingApproval = useHasPendingApproval(erc20ReserveData.debtTokenAddress, spender)
  useEffect(() => {
    if (tokenContract && address && spender) {
      tokenContract.delegateAllowance(address, spender).then((allowance: BigNumber) => {
        if (new BigNumber(amount).lte(allowance.toString())) {
          setApprovalState(ApprovalState.APPROVED)
        } else {
          setApprovalState(ApprovalState.NOT_APPROVED)
