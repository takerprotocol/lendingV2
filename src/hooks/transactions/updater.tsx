...
import useBlockNumber, { useFastForwardBlockNumber } from 'hooks/transactions/useBlockNumber'
...
const RETRY_OPTIONS_BY_CHAIN_ID: { [chainId: number]: RetryOptions } = {
  ...
  [SupportedChainId.GOERLI]: { n: 10, minWait: 250, maxWait: 500 },
}
...
export default function Updater({ pendingTransactions, onCheck, onReceipt }: UpdaterProps): null {
  ...
  const getReceipt = useCallback(
    (hash: string) => {
      ...
      return retry(
        () =>
          provider.getTransactionReceipt(hash).then((receipt: null) => {
            if (receipt === null) {
    ...
    const cancels = Object.keys(pendingTransactions)
      ...
      .map((hash) => {
        ...
        promise
          ...
          .catch((error: { isCancelledError: any }) => {
