...
const MobileWalletBox = styled(Box)`
  background: #ffffff;
...
export default function MobileWalletModal() {
  ...
  function getOptions() {
    ...
    return Object.keys(SUPPORTED_WALLETS).map((key) => {
      ...
      if (isMobile) {
        if ((true && option.type) || (!window.web3 && !window.ethereum && option.mobile)) {
          return (
            <MobileOption
              onClickEvt={() => {
  ...
  return (
    <MobileWalletBox>
      <MobileModal isTitle={!error} title="Select a wallet"></MobileModal>
      <Box padding={'0.75rem 1rem 1rem 1rem'}>{getModalContent()}</Box>
    </MobileWalletBox>
  )
}
